...
function InputCampos() {
  ...
  const adicionarValores = () => {
    ...
    if ((!isNaN(novaParede.largura) && !isNaN(novaParede.altura)) && (!isNaN(novaParede.portas) && !isNaN(novaParede.janelas))) {
      ...
      alert("Não deve haver valores nulos");
      setParedes([]);
    }
  ...
  /*Soma de todas as áreas das parede */
      ...
      <div style={{ margin: "10px 30px" }}>
        {paredes.map((parede, index) => (
          <p key={index} style={{ marginBottom: "10px", paddingRight: "35px" }}>
            Parede {index + 1}: Largura - {parede.largura.toFixed(2)} | Altura - {parede.altura.toFixed(2)} | Portas: {parede.portas} | Janelas: {parede.janelas} | Área: {((parede.largura * parede.altura).toString()).length >= 5 ? (parede.largura * parede.altura).toFixed(2) : parede.largura * parede.altura}m²
          </p>
        ))}
