...
export function InputCampos(): React.ReactElement {
  ...
  const solicitarMedidasParede = (indexParede: number): Parede | null => {
    const larguraStr = prompt(`${INSTRUCOES_LARGURA} ${indexParede}:`);
    const alturaStr = prompt(`${INSTRUCOES_ALTURA} ${indexParede}:`);
    const portasStr = prompt(`${INSTRUCOES_PORTAS} ${indexParede}:`);
    const janelasStr = prompt(`${INSTRUCOES_JANELAS} ${indexParede}:`);
